...
export function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <article className="group overflow-hidden rounded-[1.5rem] border border-white/10 bg-zinc-950/70 shadow-[0_18px_50px_rgba(0,0,0,0.22)]">
      ...
      <div className="space-y-3 p-4 sm:p-5">
        ...
        <Link href={`/product/${product.slug}`} className="block">
          <h3 className="text-lg font-semibold leading-tight text-white transition group-hover:text-zinc-300 sm:text-xl">
            {product.name}
          </h3>
        </Link>
        ...
        <div className="flex items-center justify-between">
          <div>
            <p className="text-base font-semibold text-white sm:text-lg">{formatCurrency(product.offerPrice ?? product.price)}</p>
            {product.offerPrice ? (
          ...
          <AddToCartButton
            product={product}
            className="rounded-full border border-white/10 bg-white/5 p-2.5 text-white transition hover:bg-white/10 animate-[pulse_0.45s_ease-out_1]"
